...
SCREENSHOT_DIR = Path("api-tests/screenshots/users-application-password_outputs")
...
def test_get_valid_users_application_password():
    """Test Case 1: Get valid users/(?P<user_id>(?:[\\d]+|me))/application-password"""
    # First, get list to find a valid identifier
    list_path = "/wp/v2/users/(?P<user_id>(?:[\d]+|me))/application-passwords"
    ...
    url = f"{BASE_URL}/wp/v2/users/(?P<user_id>(?:[\\d]+|me))/application-passwords/{{id}}".replace("{{id}}", str(identifier))
    try:
    ...
    save_response_screenshot("get_valid_users_application_password", response)
...
def test_get_invalid_users_application_password():
    """Test Case 2: Get invalid users/(?P<user_id>(?:[\\d]+|me))/application-password"""
    url = f"{BASE_URL}/wp/v2/users/(?P<user_id>(?:[\\d]+|me))/application-passwords/{{id}}".replace("{{id}}", "invalid-999999")
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
        pytest.skip("WordPress server is not running or not accessible")
    ...
    save_response_screenshot("get_invalid_users_application_password", response)
    
    # Accept 404 (not found) as expected, or 200 if somehow valid
    assert response.status_code in [200, 404], f"Expected 200 or 404, got {response.status_code}"

def test_unauthorized_users_application_password():
    """Test Case 3: Unauthorized access to users/(?P<user_id>(?:[\\d]+|me))/application-password"""
    url = f"{BASE_URL}/wp/v2/users/(?P<user_id>(?:[\\d]+|me))/application-passwords/{{id}}".replace("{{id}}", "test")
    try:
    ...
    save_response_screenshot("unauthorized_users/(?P<user_id>(?:[\\d]+|me))/application-password", response)
...
def test_response_schema_users_application_password():
    """Test Case 4: Response Schema Validation for users/(?P<user_id>(?:[\\d]+|me))/application-password"""
    # First, get list to find a valid identifier
    list_path = "/wp/v2/users/(?P<user_id>(?:[\d]+|me))/application-passwords"
    list_url = f"{BASE_URL}{list_path}"
    try:
    ...
    save_response_screenshot("schema_users_application_password", response)
...
def test_response_content_type_users_application_password():
    """Test Case 5: Response Content Type for users/(?P<user_id>(?:[\\d]+|me))/application-password"""
    # First, get list to find a valid identifier
    list_path = "/wp/v2/users/(?P<user_id>(?:[\d]+|me))/application-passwords"
    list_url = f"{BASE_URL}{list_path}"
    try:
    ...
    save_response_screenshot("content_type_users_application_password", response)
...
def test_response_structure_users_application_password():
    """Test Case 6: Response Structure Validation for users/(?P<user_id>(?:[\\d]+|me))/application-password"""
    # First, get list to find a valid identifier
    list_path = "/wp/v2/users/(?P<user_id>(?:[\d]+|me))/application-passwords"
    list_url = f"{BASE_URL}{list_path}"
    try:
    ...
    save_response_screenshot("structure_users_application_password", response)
