...
SCREENSHOT_DIR = Path("api-tests/screenshots/posts_outputs")
...
def test_get_all_posts():
    """Test Case 1: Retrieve all posts"""
    ...
    save_response_screenshot("get_all_posts", response)
...
def test_unauthorized_posts():
    """Test Case 2: Unauthorized access to posts"""
    url = f"{BASE_URL}/wp/v2/posts"
    try:
    ...
    save_response_screenshot("unauthorized_posts", response)
...
def test_pagination_posts():
    """Test Case 3: Pagination for posts"""
    url = f"{BASE_URL}/wp/v2/posts?page=1&per_page=5"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
        pytest.skip("WordPress server is not running or not accessible")
    ...
    save_response_screenshot("pagination_posts", response)
...
def test_response_schema_posts():
    """Test Case 4: Response Schema Validation for posts"""
    url = f"{BASE_URL}/wp/v2/posts"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
    ...
    save_response_screenshot("schema_posts", response)
...
def test_response_content_type_posts():
    """Test Case 5: Response Content Type for posts"""
    url = f"{BASE_URL}/wp/v2/posts"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
    ...
    save_response_screenshot("content_type_posts", response)
...
def test_response_structure_posts():
    """Test Case 6: Response Structure Validation for posts"""
    url = f"{BASE_URL}/wp/v2/posts"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
    ...
    save_response_screenshot("structure_posts", response)
...
def test_head_posts():
    """Test Case 7: HEAD request for posts"""
    url = f"{BASE_URL}/wp/v2/posts"
    try:
    ...
    save_response_screenshot("head_posts", response)
