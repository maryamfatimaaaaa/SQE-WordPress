...
SCREENSHOT_DIR = Path("api-tests/screenshots/widget-type_outputs")
...
def test_get_valid_widget_type():
    """Test Case 1: Get valid widget-type"""
    # First, get list to find a valid identifier
    list_path = "/wp/v2/widget-types"
    ...
    url = f"{BASE_URL}/wp/v2/widget-types/{{id}}".replace("{{id}}", str(identifier))
    try:
    ...
    save_response_screenshot("get_valid_widget_type", response)
...
def test_get_invalid_widget_type():
    """Test Case 2: Get invalid widget-type"""
    url = f"{BASE_URL}/wp/v2/widget-types/{{id}}".replace("{{id}}", "invalid-999999")
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
        pytest.skip("WordPress server is not running or not accessible")
    ...
    save_response_screenshot("get_invalid_widget_type", response)
    
    # Accept 404 (not found) as expected, or 200 if somehow valid
    assert response.status_code in [200, 404], f"Expected 200 or 404, got {response.status_code}"

def test_unauthorized_widget_type():
    """Test Case 3: Unauthorized access to widget-type"""
    url = f"{BASE_URL}/wp/v2/widget-types/{{id}}".replace("{{id}}", "test")
    try:
    ...
    save_response_screenshot("unauthorized_widget-type", response)
...
def test_response_schema_widget_type():
    """Test Case 4: Response Schema Validation for widget-type"""
    # First, get list to find a valid identifier
    list_path = "/wp/v2/widget-types"
    list_url = f"{BASE_URL}{list_path}"
    try:
    ...
    save_response_screenshot("schema_widget_type", response)
...
def test_response_content_type_widget_type():
    """Test Case 5: Response Content Type for widget-type"""
    # First, get list to find a valid identifier
    list_path = "/wp/v2/widget-types"
    list_url = f"{BASE_URL}{list_path}"
    try:
    ...
    save_response_screenshot("content_type_widget_type", response)
...
def test_response_structure_widget_type():
    """Test Case 6: Response Structure Validation for widget-type"""
    # First, get list to find a valid identifier
    list_path = "/wp/v2/widget-types"
    list_url = f"{BASE_URL}{list_path}"
    try:
    ...
    save_response_screenshot("structure_widget_type", response)
