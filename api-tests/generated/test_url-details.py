...
SCREENSHOT_DIR = Path("api-tests/screenshots/url-details_outputs")
...
def test_get_valid_url_details():
    """Test Case 1: Get valid url-details"""
    # First, get list to find a valid identifier
    list_path = "/wp-block-editor/v1/url-details"
    ...
    url = f"{BASE_URL}/wp-block-editor/v1/url-details/{{id}}".replace("{{id}}", str(identifier))
    try:
    ...
    save_response_screenshot("get_valid_url_details", response)
...
def test_get_invalid_url_details():
    """Test Case 2: Get invalid url-details"""
    url = f"{BASE_URL}/wp-block-editor/v1/url-details/{{id}}".replace("{{id}}", "invalid-999999")
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
        pytest.skip("WordPress server is not running or not accessible")
    ...
    save_response_screenshot("get_invalid_url_details", response)
    
    # Accept 404 (not found) as expected, or 200 if somehow valid
    assert response.status_code in [200, 404], f"Expected 200 or 404, got {response.status_code}"

def test_unauthorized_url_details():
    """Test Case 3: Unauthorized access to url-details"""
    url = f"{BASE_URL}/wp-block-editor/v1/url-details/{{id}}".replace("{{id}}", "test")
    try:
    ...
    save_response_screenshot("unauthorized_url-details", response)
...
def test_response_schema_url_details():
    """Test Case 4: Response Schema Validation for url-details"""
    # First, get list to find a valid identifier
    list_path = "/wp-block-editor/v1/url-details"
    list_url = f"{BASE_URL}{list_path}"
    try:
    ...
    save_response_screenshot("schema_url_details", response)
...
def test_response_content_type_url_details():
    """Test Case 5: Response Content Type for url-details"""
    # First, get list to find a valid identifier
    list_path = "/wp-block-editor/v1/url-details"
    list_url = f"{BASE_URL}{list_path}"
    try:
    ...
    save_response_screenshot("content_type_url_details", response)
...
def test_response_structure_url_details():
    """Test Case 6: Response Structure Validation for url-details"""
    # First, get list to find a valid identifier
    list_path = "/wp-block-editor/v1/url-details"
    list_url = f"{BASE_URL}{list_path}"
    try:
    ...
    save_response_screenshot("structure_url_details", response)
