...
SCREENSHOT_DIR = Path("api-tests/screenshots/menu-item_outputs")
...
def test_get_valid_menu_item():
    """Test Case 1: Get valid menu-item"""
    # First, get list to find a valid identifier
    list_path = "/wp/v2/menu-items"
    ...
    url = f"{BASE_URL}/wp/v2/menu-items/{{id}}".replace("{{id}}", str(identifier))
    try:
    ...
    save_response_screenshot("get_valid_menu_item", response)
...
def test_get_invalid_menu_item():
    """Test Case 2: Get invalid menu-item"""
    url = f"{BASE_URL}/wp/v2/menu-items/{{id}}".replace("{{id}}", "invalid-999999")
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
        pytest.skip("WordPress server is not running or not accessible")
    ...
    save_response_screenshot("get_invalid_menu_item", response)
    
    # Accept 404 (not found) as expected, or 200 if somehow valid
    assert response.status_code in [200, 404], f"Expected 200 or 404, got {response.status_code}"

def test_unauthorized_menu_item():
    """Test Case 3: Unauthorized access to menu-item"""
    url = f"{BASE_URL}/wp/v2/menu-items/{{id}}".replace("{{id}}", "test")
    try:
    ...
    save_response_screenshot("unauthorized_menu-item", response)
...
def test_response_schema_menu_item():
    """Test Case 4: Response Schema Validation for menu-item"""
    # First, get list to find a valid identifier
    list_path = "/wp/v2/menu-items"
    list_url = f"{BASE_URL}{list_path}"
    try:
    ...
    save_response_screenshot("schema_menu_item", response)
...
def test_response_content_type_menu_item():
    """Test Case 5: Response Content Type for menu-item"""
    # First, get list to find a valid identifier
    list_path = "/wp/v2/menu-items"
    list_url = f"{BASE_URL}{list_path}"
    try:
    ...
    save_response_screenshot("content_type_menu_item", response)
...
def test_response_structure_menu_item():
    """Test Case 6: Response Structure Validation for menu-item"""
    # First, get list to find a valid identifier
    list_path = "/wp/v2/menu-items"
    list_url = f"{BASE_URL}{list_path}"
    try:
    ...
    save_response_screenshot("structure_menu_item", response)
