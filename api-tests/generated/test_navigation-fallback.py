...
SCREENSHOT_DIR = Path("api-tests/screenshots/navigation-fallback_outputs")
...
def test_get_valid_navigation_fallback():
    """Test Case 1: Get valid navigation-fallback"""
    # First, get list to find a valid identifier
    list_path = "/wp-block-editor/v1/navigation-fallback"
    ...
    url = f"{BASE_URL}/wp-block-editor/v1/navigation-fallback/{{id}}".replace("{{id}}", str(identifier))
    try:
    ...
    save_response_screenshot("get_valid_navigation_fallback", response)
...
def test_get_invalid_navigation_fallback():
    """Test Case 2: Get invalid navigation-fallback"""
    url = f"{BASE_URL}/wp-block-editor/v1/navigation-fallback/{{id}}".replace("{{id}}", "invalid-999999")
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
        pytest.skip("WordPress server is not running or not accessible")
    ...
    save_response_screenshot("get_invalid_navigation_fallback", response)
    
    # Accept 404 (not found) as expected, or 200 if somehow valid
    assert response.status_code in [200, 404], f"Expected 200 or 404, got {response.status_code}"

def test_unauthorized_navigation_fallback():
    """Test Case 3: Unauthorized access to navigation-fallback"""
    url = f"{BASE_URL}/wp-block-editor/v1/navigation-fallback/{{id}}".replace("{{id}}", "test")
    try:
    ...
    save_response_screenshot("unauthorized_navigation-fallback", response)
...
def test_response_schema_navigation_fallback():
    """Test Case 4: Response Schema Validation for navigation-fallback"""
    # First, get list to find a valid identifier
    list_path = "/wp-block-editor/v1/navigation-fallback"
    list_url = f"{BASE_URL}{list_path}"
    try:
    ...
    save_response_screenshot("schema_navigation_fallback", response)
...
def test_response_content_type_navigation_fallback():
    """Test Case 5: Response Content Type for navigation-fallback"""
    # First, get list to find a valid identifier
    list_path = "/wp-block-editor/v1/navigation-fallback"
    list_url = f"{BASE_URL}{list_path}"
    try:
    ...
    save_response_screenshot("content_type_navigation_fallback", response)
...
def test_response_structure_navigation_fallback():
    """Test Case 6: Response Structure Validation for navigation-fallback"""
    # First, get list to find a valid identifier
    list_path = "/wp-block-editor/v1/navigation-fallback"
    list_url = f"{BASE_URL}{list_path}"
    try:
    ...
    save_response_screenshot("structure_navigation_fallback", response)
