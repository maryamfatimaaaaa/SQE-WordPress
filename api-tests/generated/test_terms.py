...
SCREENSHOT_DIR = Path("api-tests/screenshots/terms_outputs")
...
def test_get_all_terms():
    """Test Case 1: Retrieve all terms"""
    url = f"{BASE_URL}/wp/v2/terms"
    try:
    ...
    save_response_screenshot("get_all_terms", response)
...
def test_unauthorized_terms():
    """Test Case 2: Unauthorized access to terms"""
    url = f"{BASE_URL}/wp/v2/terms"
    try:
    ...
    save_response_screenshot("unauthorized_terms", response)
...
def test_pagination_terms():
    """Test Case 3: Pagination for terms"""
    url = f"{BASE_URL}/wp/v2/terms?page=1&per_page=5"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
        pytest.skip("WordPress server is not running or not accessible")
    ...
    save_response_screenshot("pagination_terms", response)
...
def test_response_schema_terms():
    """Test Case 4: Response Schema Validation for terms"""
    url = f"{BASE_URL}/wp/v2/terms"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
    ...
    save_response_screenshot("schema_terms", response)
...
def test_response_content_type_terms():
    """Test Case 5: Response Content Type for terms"""
    url = f"{BASE_URL}/wp/v2/terms"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
    ...
    save_response_screenshot("content_type_terms", response)
...
def test_response_structure_terms():
    """Test Case 6: Response Structure Validation for terms"""
    url = f"{BASE_URL}/wp/v2/terms"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
    ...
    save_response_screenshot("structure_terms", response)
...
def test_head_terms():
    """Test Case 7: HEAD request for terms"""
    url = f"{BASE_URL}/wp/v2/terms"
    try:
    ...
    save_response_screenshot("head_terms", response)
