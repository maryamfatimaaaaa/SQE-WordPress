...
SCREENSHOT_DIR = Path("api-tests/screenshots/categorie_outputs")
...
def test_get_valid_categorie():
    """Test Case 1: Get valid categorie"""
    # First, get list to find a valid identifier
    list_path = "/wp-abilities/v1/categories"
    ...
    url = f"{BASE_URL}/wp-abilities/v1/categories/{{slug}}".replace("{{slug}}", str(identifier))
    try:
    ...
    save_response_screenshot("get_valid_categorie", response)
...
def test_get_invalid_categorie():
    """Test Case 2: Get invalid categorie"""
    url = f"{BASE_URL}/wp-abilities/v1/categories/{{slug}}".replace("{{slug}}", "invalid-999999")
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
        pytest.skip("WordPress server is not running or not accessible")
    ...
    save_response_screenshot("get_invalid_categorie", response)
    
    # Accept 404 (not found) as expected, or 200 if somehow valid
    assert response.status_code in [200, 404], f"Expected 200 or 404, got {response.status_code}"

def test_unauthorized_categorie():
    """Test Case 3: Unauthorized access to categorie"""
    url = f"{BASE_URL}/wp-abilities/v1/categories/{{slug}}".replace("{{slug}}", "test")
    try:
    ...
    save_response_screenshot("unauthorized_categorie", response)
...
def test_response_schema_categorie():
    """Test Case 4: Response Schema Validation for categorie"""
    # First, get list to find a valid identifier
    list_path = "/wp-abilities/v1/categories"
    list_url = f"{BASE_URL}{list_path}"
    try:
    ...
    save_response_screenshot("schema_categorie", response)
...
def test_response_content_type_categorie():
    """Test Case 5: Response Content Type for categorie"""
    # First, get list to find a valid identifier
    list_path = "/wp-abilities/v1/categories"
    list_url = f"{BASE_URL}{list_path}"
    try:
    ...
    save_response_screenshot("content_type_categorie", response)
...
def test_response_structure_categorie():
    """Test Case 6: Response Structure Validation for categorie"""
    # First, get list to find a valid identifier
    list_path = "/wp-abilities/v1/categories"
    list_url = f"{BASE_URL}{list_path}"
    try:
    ...
    save_response_screenshot("structure_categorie", response)
