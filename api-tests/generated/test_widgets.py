...
SCREENSHOT_DIR = Path("api-tests/screenshots/widgets_outputs")
...
def test_get_all_widgets():
    """Test Case 1: Retrieve all widgets"""
    url = f"{BASE_URL}/wp/v2/widgets"
    try:
    ...
    save_response_screenshot("get_all_widgets", response)
...
def test_unauthorized_widgets():
    """Test Case 2: Unauthorized access to widgets"""
    url = f"{BASE_URL}/wp/v2/widgets"
    try:
    ...
    save_response_screenshot("unauthorized_widgets", response)
...
def test_pagination_widgets():
    """Test Case 3: Pagination for widgets"""
    url = f"{BASE_URL}/wp/v2/widgets?page=1&per_page=5"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
        pytest.skip("WordPress server is not running or not accessible")
    ...
    save_response_screenshot("pagination_widgets", response)
...
def test_response_schema_widgets():
    """Test Case 4: Response Schema Validation for widgets"""
    url = f"{BASE_URL}/wp/v2/widgets"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
    ...
    save_response_screenshot("schema_widgets", response)
...
def test_response_content_type_widgets():
    """Test Case 5: Response Content Type for widgets"""
    url = f"{BASE_URL}/wp/v2/widgets"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
    ...
    save_response_screenshot("content_type_widgets", response)
...
def test_response_structure_widgets():
    """Test Case 6: Response Structure Validation for widgets"""
    url = f"{BASE_URL}/wp/v2/widgets"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
    ...
    save_response_screenshot("structure_widgets", response)
...
def test_head_widgets():
    """Test Case 7: HEAD request for widgets"""
    url = f"{BASE_URL}/wp/v2/widgets"
    try:
    ...
    save_response_screenshot("head_widgets", response)
