...
SCREENSHOT_DIR = Path("api-tests/screenshots/themes_outputs")
...
def test_get_all_themes():
    """Test Case 1: Retrieve all themes"""
    url = f"{BASE_URL}/wp/v2/themes"
    try:
    ...
    save_response_screenshot("get_all_themes", response)
...
def test_unauthorized_themes():
    """Test Case 2: Unauthorized access to themes"""
    url = f"{BASE_URL}/wp/v2/themes"
    try:
    ...
    save_response_screenshot("unauthorized_themes", response)
...
def test_pagination_themes():
    """Test Case 3: Pagination for themes"""
    url = f"{BASE_URL}/wp/v2/themes?page=1&per_page=5"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
        pytest.skip("WordPress server is not running or not accessible")
    ...
    save_response_screenshot("pagination_themes", response)
...
def test_response_schema_themes():
    """Test Case 4: Response Schema Validation for themes"""
    url = f"{BASE_URL}/wp/v2/themes"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
    ...
    save_response_screenshot("schema_themes", response)
...
def test_response_content_type_themes():
    """Test Case 5: Response Content Type for themes"""
    url = f"{BASE_URL}/wp/v2/themes"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
    ...
    save_response_screenshot("content_type_themes", response)
...
def test_response_structure_themes():
    """Test Case 6: Response Structure Validation for themes"""
    url = f"{BASE_URL}/wp/v2/themes"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
    ...
    save_response_screenshot("structure_themes", response)
...
def test_head_themes():
    """Test Case 7: HEAD request for themes"""
    url = f"{BASE_URL}/wp/v2/themes"
    try:
    ...
    save_response_screenshot("head_themes", response)
