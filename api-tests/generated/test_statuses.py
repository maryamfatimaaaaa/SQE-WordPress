...
SCREENSHOT_DIR = Path("api-tests/screenshots/statuses_outputs")
...
def test_get_all_statuses():
    """Test Case 1: Retrieve all statuses"""
    url = f"{BASE_URL}/wp/v2/statuses"
    try:
    ...
    save_response_screenshot("get_all_statuses", response)
...
def test_unauthorized_statuses():
    """Test Case 2: Unauthorized access to statuses"""
    url = f"{BASE_URL}/wp/v2/statuses"
    try:
    ...
    save_response_screenshot("unauthorized_statuses", response)
...
def test_pagination_statuses():
    """Test Case 3: Pagination for statuses"""
    url = f"{BASE_URL}/wp/v2/statuses?page=1&per_page=5"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
        pytest.skip("WordPress server is not running or not accessible")
    ...
    save_response_screenshot("pagination_statuses", response)
...
def test_response_schema_statuses():
    """Test Case 4: Response Schema Validation for statuses"""
    url = f"{BASE_URL}/wp/v2/statuses"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
    ...
    save_response_screenshot("schema_statuses", response)
...
def test_response_content_type_statuses():
    """Test Case 5: Response Content Type for statuses"""
    url = f"{BASE_URL}/wp/v2/statuses"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
    ...
    save_response_screenshot("content_type_statuses", response)
...
def test_response_structure_statuses():
    """Test Case 6: Response Structure Validation for statuses"""
    url = f"{BASE_URL}/wp/v2/statuses"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
    ...
    save_response_screenshot("structure_statuses", response)
...
def test_head_statuses():
    """Test Case 7: HEAD request for statuses"""
    url = f"{BASE_URL}/wp/v2/statuses"
    try:
    ...
    save_response_screenshot("head_statuses", response)
