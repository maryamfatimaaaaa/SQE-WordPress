...
SCREENSHOT_DIR = Path("api-tests/screenshots/templates_outputs")
...
def test_get_all_templates():
    """Test Case 1: Retrieve all templates"""
    url = f"{BASE_URL}/wp/v2/templates"
    try:
    ...
    save_response_screenshot("get_all_templates", response)
...
def test_unauthorized_templates():
    """Test Case 2: Unauthorized access to templates"""
    url = f"{BASE_URL}/wp/v2/templates"
    try:
    ...
    save_response_screenshot("unauthorized_templates", response)
...
def test_pagination_templates():
    """Test Case 3: Pagination for templates"""
    url = f"{BASE_URL}/wp/v2/templates?page=1&per_page=5"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
        pytest.skip("WordPress server is not running or not accessible")
    ...
    save_response_screenshot("pagination_templates", response)
...
def test_response_schema_templates():
    """Test Case 4: Response Schema Validation for templates"""
    url = f"{BASE_URL}/wp/v2/templates"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
    ...
    save_response_screenshot("schema_templates", response)
...
def test_response_content_type_templates():
    """Test Case 5: Response Content Type for templates"""
    url = f"{BASE_URL}/wp/v2/templates"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
    ...
    save_response_screenshot("content_type_templates", response)
...
def test_response_structure_templates():
    """Test Case 6: Response Structure Validation for templates"""
    url = f"{BASE_URL}/wp/v2/templates"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
    ...
    save_response_screenshot("structure_templates", response)
...
def test_head_templates():
    """Test Case 7: HEAD request for templates"""
    url = f"{BASE_URL}/wp/v2/templates"
    try:
    ...
    save_response_screenshot("head_templates", response)
