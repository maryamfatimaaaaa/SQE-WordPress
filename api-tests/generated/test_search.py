...
SCREENSHOT_DIR = Path("api-tests/screenshots/search_outputs")
...
def test_get_valid_search():
    """Test Case 1: Get valid search"""
    # First, get list to find a valid identifier
    list_path = "/wp/v2/search"
    ...
    url = f"{BASE_URL}/wp/v2/search/{{id}}".replace("{{id}}", str(identifier))
    try:
    ...
    save_response_screenshot("get_valid_search", response)
...
def test_get_invalid_search():
    """Test Case 2: Get invalid search"""
    url = f"{BASE_URL}/wp/v2/search/{{id}}".replace("{{id}}", "invalid-999999")
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
        pytest.skip("WordPress server is not running or not accessible")
    ...
    save_response_screenshot("get_invalid_search", response)
    
    # Accept 404 (not found) as expected, or 200 if somehow valid
    assert response.status_code in [200, 404], f"Expected 200 or 404, got {response.status_code}"

def test_unauthorized_search():
    """Test Case 3: Unauthorized access to search"""
    url = f"{BASE_URL}/wp/v2/search/{{id}}".replace("{{id}}", "test")
    try:
    ...
    save_response_screenshot("unauthorized_search", response)
...
def test_response_schema_search():
    """Test Case 4: Response Schema Validation for search"""
    # First, get list to find a valid identifier
    list_path = "/wp/v2/search"
    list_url = f"{BASE_URL}{list_path}"
    try:
    ...
    save_response_screenshot("schema_search", response)
...
def test_response_content_type_search():
    """Test Case 5: Response Content Type for search"""
    # First, get list to find a valid identifier
    list_path = "/wp/v2/search"
    list_url = f"{BASE_URL}{list_path}"
    try:
    ...
    save_response_screenshot("content_type_search", response)
...
def test_response_structure_search():
    """Test Case 6: Response Structure Validation for search"""
    # First, get list to find a valid identifier
    list_path = "/wp/v2/search"
    list_url = f"{BASE_URL}{list_path}"
    try:
    ...
    save_response_screenshot("structure_search", response)
