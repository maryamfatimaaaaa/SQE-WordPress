...
SCREENSHOT_DIR = Path("api-tests/screenshots/attachments_outputs")
...
def test_get_all_attachments():
    """Test Case 1: Retrieve all attachments"""
    url = f"{BASE_URL}/wp/v2/attachments"
    try:
    ...
    save_response_screenshot("get_all_attachments", response)
...
def test_unauthorized_attachments():
    """Test Case 2: Unauthorized access to attachments"""
    url = f"{BASE_URL}/wp/v2/attachments"
    try:
    ...
    save_response_screenshot("unauthorized_attachments", response)
...
def test_pagination_attachments():
    """Test Case 3: Pagination for attachments"""
    url = f"{BASE_URL}/wp/v2/attachments?page=1&per_page=5"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
        pytest.skip("WordPress server is not running or not accessible")
    ...
    save_response_screenshot("pagination_attachments", response)
...
def test_response_schema_attachments():
    """Test Case 4: Response Schema Validation for attachments"""
    url = f"{BASE_URL}/wp/v2/attachments"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
    ...
    save_response_screenshot("schema_attachments", response)
...
def test_response_content_type_attachments():
    """Test Case 5: Response Content Type for attachments"""
    url = f"{BASE_URL}/wp/v2/attachments"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
    ...
    save_response_screenshot("content_type_attachments", response)
...
def test_response_structure_attachments():
    """Test Case 6: Response Structure Validation for attachments"""
    url = f"{BASE_URL}/wp/v2/attachments"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
    ...
    save_response_screenshot("structure_attachments", response)
...
def test_head_attachments():
    """Test Case 7: HEAD request for attachments"""
    url = f"{BASE_URL}/wp/v2/attachments"
    try:
    ...
    save_response_screenshot("head_attachments", response)
