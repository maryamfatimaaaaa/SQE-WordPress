...
SCREENSHOT_DIR = Path("api-tests/screenshots/plugins_outputs")
...
def test_get_all_plugins():
    """Test Case 1: Retrieve all plugins"""
    url = f"{BASE_URL}/wp/v2/plugins"
    try:
    ...
    save_response_screenshot("get_all_plugins", response)
...
def test_unauthorized_plugins():
    """Test Case 2: Unauthorized access to plugins"""
    url = f"{BASE_URL}/wp/v2/plugins"
    try:
    ...
    save_response_screenshot("unauthorized_plugins", response)
...
def test_pagination_plugins():
    """Test Case 3: Pagination for plugins"""
    url = f"{BASE_URL}/wp/v2/plugins?page=1&per_page=5"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
        pytest.skip("WordPress server is not running or not accessible")
    ...
    save_response_screenshot("pagination_plugins", response)
...
def test_response_schema_plugins():
    """Test Case 4: Response Schema Validation for plugins"""
    url = f"{BASE_URL}/wp/v2/plugins"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
    ...
    save_response_screenshot("schema_plugins", response)
...
def test_response_content_type_plugins():
    """Test Case 5: Response Content Type for plugins"""
    url = f"{BASE_URL}/wp/v2/plugins"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
    ...
    save_response_screenshot("content_type_plugins", response)
...
def test_response_structure_plugins():
    """Test Case 6: Response Structure Validation for plugins"""
    url = f"{BASE_URL}/wp/v2/plugins"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
    ...
    save_response_screenshot("structure_plugins", response)
...
def test_head_plugins():
    """Test Case 7: HEAD request for plugins"""
    url = f"{BASE_URL}/wp/v2/plugins"
    try:
    ...
    save_response_screenshot("head_plugins", response)
