...
SCREENSHOT_DIR = Path("api-tests/screenshots/tests_outputs")
...
def test_get_valid_tests():
    """Test Case 1: Get valid tests"""
    # First, get list to find a valid identifier
    list_path = "/wp-site-health/v1/tests"
    ...
    url = f"{BASE_URL}/wp-site-health/v1/tests/{{id}}".replace("{{id}}", str(identifier))
    try:
    ...
    save_response_screenshot("get_valid_tests", response)
...
def test_get_invalid_tests():
    """Test Case 2: Get invalid tests"""
    url = f"{BASE_URL}/wp-site-health/v1/tests/{{id}}".replace("{{id}}", "invalid-999999")
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
        pytest.skip("WordPress server is not running or not accessible")
    ...
    save_response_screenshot("get_invalid_tests", response)
    
    # Accept 404 (not found) as expected, or 200 if somehow valid
    assert response.status_code in [200, 404], f"Expected 200 or 404, got {response.status_code}"

def test_unauthorized_tests():
    """Test Case 3: Unauthorized access to tests"""
    url = f"{BASE_URL}/wp-site-health/v1/tests/{{id}}".replace("{{id}}", "test")
    try:
    ...
    save_response_screenshot("unauthorized_tests", response)
...
def test_response_schema_tests():
    """Test Case 4: Response Schema Validation for tests"""
    # First, get list to find a valid identifier
    list_path = "/wp-site-health/v1/tests"
    list_url = f"{BASE_URL}{list_path}"
    try:
    ...
    save_response_screenshot("schema_tests", response)
...
def test_response_content_type_tests():
    """Test Case 5: Response Content Type for tests"""
    # First, get list to find a valid identifier
    list_path = "/wp-site-health/v1/tests"
    list_url = f"{BASE_URL}{list_path}"
    try:
    ...
    save_response_screenshot("content_type_tests", response)
...
def test_response_structure_tests():
    """Test Case 6: Response Structure Validation for tests"""
    # First, get list to find a valid identifier
    list_path = "/wp-site-health/v1/tests"
    list_url = f"{BASE_URL}{list_path}"
    try:
    ...
    save_response_screenshot("structure_tests", response)
