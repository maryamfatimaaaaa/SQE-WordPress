...
SCREENSHOT_DIR = Path("api-tests/screenshots/sidebars_outputs")
...
def test_get_all_sidebars():
    """Test Case 1: Retrieve all sidebars"""
    url = f"{BASE_URL}/wp/v2/sidebars"
    try:
    ...
    save_response_screenshot("get_all_sidebars", response)
...
def test_unauthorized_sidebars():
    """Test Case 2: Unauthorized access to sidebars"""
    url = f"{BASE_URL}/wp/v2/sidebars"
    try:
    ...
    save_response_screenshot("unauthorized_sidebars", response)
...
def test_pagination_sidebars():
    """Test Case 3: Pagination for sidebars"""
    url = f"{BASE_URL}/wp/v2/sidebars?page=1&per_page=5"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
        pytest.skip("WordPress server is not running or not accessible")
    ...
    save_response_screenshot("pagination_sidebars", response)
...
def test_response_schema_sidebars():
    """Test Case 4: Response Schema Validation for sidebars"""
    url = f"{BASE_URL}/wp/v2/sidebars"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
    ...
    save_response_screenshot("schema_sidebars", response)
...
def test_response_content_type_sidebars():
    """Test Case 5: Response Content Type for sidebars"""
    url = f"{BASE_URL}/wp/v2/sidebars"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
    ...
    save_response_screenshot("content_type_sidebars", response)
...
def test_response_structure_sidebars():
    """Test Case 6: Response Structure Validation for sidebars"""
    url = f"{BASE_URL}/wp/v2/sidebars"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
    ...
    save_response_screenshot("structure_sidebars", response)
...
def test_head_sidebars():
    """Test Case 7: HEAD request for sidebars"""
    url = f"{BASE_URL}/wp/v2/sidebars"
    try:
    ...
    save_response_screenshot("head_sidebars", response)
