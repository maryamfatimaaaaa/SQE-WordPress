...
SCREENSHOT_DIR = Path("api-tests/screenshots/revisions_outputs")
...
def test_get_all_revisions():
    """Test Case 1: Retrieve all revisions"""
    url = f"{BASE_URL}/wp/v2/revisions"
    try:
    ...
    save_response_screenshot("get_all_revisions", response)
...
def test_unauthorized_revisions():
    """Test Case 2: Unauthorized access to revisions"""
    url = f"{BASE_URL}/wp/v2/revisions"
    try:
    ...
    save_response_screenshot("unauthorized_revisions", response)
...
def test_pagination_revisions():
    """Test Case 3: Pagination for revisions"""
    url = f"{BASE_URL}/wp/v2/revisions?page=1&per_page=5"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
        pytest.skip("WordPress server is not running or not accessible")
    ...
    save_response_screenshot("pagination_revisions", response)
...
def test_response_schema_revisions():
    """Test Case 4: Response Schema Validation for revisions"""
    url = f"{BASE_URL}/wp/v2/revisions"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
    ...
    save_response_screenshot("schema_revisions", response)
...
def test_response_content_type_revisions():
    """Test Case 5: Response Content Type for revisions"""
    url = f"{BASE_URL}/wp/v2/revisions"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
    ...
    save_response_screenshot("content_type_revisions", response)
...
def test_response_structure_revisions():
    """Test Case 6: Response Structure Validation for revisions"""
    url = f"{BASE_URL}/wp/v2/revisions"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
    ...
    save_response_screenshot("structure_revisions", response)
...
def test_head_revisions():
    """Test Case 7: HEAD request for revisions"""
    url = f"{BASE_URL}/wp/v2/revisions"
    try:
    ...
    save_response_screenshot("head_revisions", response)
