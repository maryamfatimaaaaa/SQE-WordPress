...
SCREENSHOT_DIR = Path("api-tests/screenshots/users_outputs")
...
def test_get_all_users():
    ...
    save_response_screenshot("get_all_users", response)
...
def test_unauthorized_users():
    """Test Case 2: Unauthorized access to users"""
    url = f"{BASE_URL}/wp/v2/users"
    try:
    ...
    save_response_screenshot("unauthorized_users", response)
...
def test_pagination_users():
    """Test Case 3: Pagination for users"""
    url = f"{BASE_URL}/wp/v2/users?page=1&per_page=5"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
        pytest.skip("WordPress server is not running or not accessible")
    ...
    save_response_screenshot("pagination_users", response)
...
def test_response_schema_users():
    """Test Case 4: Response Schema Validation for users"""
    url = f"{BASE_URL}/wp/v2/users"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
    ...
    save_response_screenshot("schema_users", response)
...
def test_response_content_type_users():
    """Test Case 5: Response Content Type for users"""
    url = f"{BASE_URL}/wp/v2/users"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
    ...
    save_response_screenshot("content_type_users", response)
...
def test_response_structure_users():
    """Test Case 6: Response Structure Validation for users"""
    url = f"{BASE_URL}/wp/v2/users"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
    ...
    save_response_screenshot("structure_users", response)
...
def test_head_users():
    """Test Case 7: HEAD request for users"""
    url = f"{BASE_URL}/wp/v2/users"
    try:
    ...
    save_response_screenshot("head_users", response)
