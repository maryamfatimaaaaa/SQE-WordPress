...
SCREENSHOT_DIR = Path("api-tests/screenshots/sidebar_outputs")
...
def test_get_valid_sidebar():
    """Test Case 1: Get valid sidebar"""
    # First, get list to find a valid identifier
    list_path = "/wp/v2/sidebars"
    ...
    url = f"{BASE_URL}/wp/v2/sidebars/{{id}}".replace("{{id}}", str(identifier))
    try:
    ...
    save_response_screenshot("get_valid_sidebar", response)
...
def test_get_invalid_sidebar():
    """Test Case 2: Get invalid sidebar"""
    url = f"{BASE_URL}/wp/v2/sidebars/{{id}}".replace("{{id}}", "invalid-999999")
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
        pytest.skip("WordPress server is not running or not accessible")
    ...
    save_response_screenshot("get_invalid_sidebar", response)
    
    # Accept 404 (not found) as expected, or 200 if somehow valid
    assert response.status_code in [200, 404], f"Expected 200 or 404, got {response.status_code}"

def test_unauthorized_sidebar():
    """Test Case 3: Unauthorized access to sidebar"""
    url = f"{BASE_URL}/wp/v2/sidebars/{{id}}".replace("{{id}}", "test")
    try:
    ...
    save_response_screenshot("unauthorized_sidebar", response)
...
def test_response_schema_sidebar():
    """Test Case 4: Response Schema Validation for sidebar"""
    # First, get list to find a valid identifier
    list_path = "/wp/v2/sidebars"
    list_url = f"{BASE_URL}{list_path}"
    try:
    ...
    save_response_screenshot("schema_sidebar", response)
...
def test_response_content_type_sidebar():
    """Test Case 5: Response Content Type for sidebar"""
    # First, get list to find a valid identifier
    list_path = "/wp/v2/sidebars"
    list_url = f"{BASE_URL}{list_path}"
    try:
    ...
    save_response_screenshot("content_type_sidebar", response)
...
def test_response_structure_sidebar():
    """Test Case 6: Response Structure Validation for sidebar"""
    # First, get list to find a valid identifier
    list_path = "/wp/v2/sidebars"
    list_url = f"{BASE_URL}{list_path}"
    try:
    ...
    save_response_screenshot("structure_sidebar", response)
