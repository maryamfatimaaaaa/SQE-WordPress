...
SCREENSHOT_DIR = Path("api-tests/screenshots/comments_outputs")
...
def test_get_all_comments():
    """Test Case 1: Retrieve all comments"""
    url = f"{BASE_URL}/wp/v2/comments"
    try:
    ...
    save_response_screenshot("get_all_comments", response)
...
def test_unauthorized_comments():
    """Test Case 2: Unauthorized access to comments"""
    url = f"{BASE_URL}/wp/v2/comments"
    try:
    ...
    save_response_screenshot("unauthorized_comments", response)
...
def test_pagination_comments():
    """Test Case 3: Pagination for comments"""
    url = f"{BASE_URL}/wp/v2/comments?page=1&per_page=5"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
        pytest.skip("WordPress server is not running or not accessible")
    ...
    save_response_screenshot("pagination_comments", response)
...
def test_response_schema_comments():
    """Test Case 4: Response Schema Validation for comments"""
    url = f"{BASE_URL}/wp/v2/comments"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
    ...
    save_response_screenshot("schema_comments", response)
...
def test_response_content_type_comments():
    """Test Case 5: Response Content Type for comments"""
    url = f"{BASE_URL}/wp/v2/comments"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
    ...
    save_response_screenshot("content_type_comments", response)
...
def test_response_structure_comments():
    """Test Case 6: Response Structure Validation for comments"""
    url = f"{BASE_URL}/wp/v2/comments"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
    ...
    save_response_screenshot("structure_comments", response)
...
def test_head_comments():
    """Test Case 7: HEAD request for comments"""
    url = f"{BASE_URL}/wp/v2/comments"
    try:
    ...
    save_response_screenshot("head_comments", response)
