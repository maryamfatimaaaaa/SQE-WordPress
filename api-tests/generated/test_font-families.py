...
SCREENSHOT_DIR = Path("api-tests/screenshots/font-families_outputs")
...
def test_get_all_font_families():
    """Test Case 1: Retrieve all font-families"""
    url = f"{BASE_URL}/wp/v2/font-families"
    try:
    ...
    save_response_screenshot("get_all_font_families", response)
...
def test_unauthorized_font_families():
    """Test Case 2: Unauthorized access to font-families"""
    url = f"{BASE_URL}/wp/v2/font-families"
    try:
    ...
    save_response_screenshot("unauthorized_font_families", response)
...
def test_pagination_font_families():
    """Test Case 3: Pagination for font-families"""
    url = f"{BASE_URL}/wp/v2/font-families?page=1&per_page=5"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
        pytest.skip("WordPress server is not running or not accessible")
    ...
    save_response_screenshot("pagination_font_families", response)
...
def test_response_schema_font_families():
    """Test Case 4: Response Schema Validation for font-families"""
    url = f"{BASE_URL}/wp/v2/font-families"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
    ...
    save_response_screenshot("schema_font_families", response)
...
def test_response_content_type_font_families():
    """Test Case 5: Response Content Type for font-families"""
    url = f"{BASE_URL}/wp/v2/font-families"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
    ...
    save_response_screenshot("content_type_font_families", response)
...
def test_response_structure_font_families():
    """Test Case 6: Response Structure Validation for font-families"""
    url = f"{BASE_URL}/wp/v2/font-families"
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
    ...
    save_response_screenshot("structure_font_families", response)
...
def test_head_font_families():
    """Test Case 7: HEAD request for font-families"""
    url = f"{BASE_URL}/wp/v2/font-families"
    try:
    ...
    save_response_screenshot("head_font_families", response)
