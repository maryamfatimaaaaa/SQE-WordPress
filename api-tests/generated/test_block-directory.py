...
SCREENSHOT_DIR = Path("api-tests/screenshots/block-directory_outputs")
...
def test_get_valid_block_directory():
    """Test Case 1: Get valid block-directory"""
    # First, get list to find a valid identifier
    list_path = "/wp/v2/block-directory"
    ...
    url = f"{BASE_URL}/wp/v2/block-directory/{{id}}".replace("{{id}}", str(identifier))
    try:
    ...
    save_response_screenshot("get_valid_block_directory", response)
...
def test_get_invalid_block_directory():
    """Test Case 2: Get invalid block-directory"""
    url = f"{BASE_URL}/wp/v2/block-directory/{{id}}".replace("{{id}}", "invalid-999999")
    try:
        response = requests.get(url, auth=HTTPBasicAuth(USERNAME, APP_PASSWORD), timeout=10)
    except requests.exceptions.ConnectionError:
        pytest.skip("WordPress server is not running or not accessible")
    ...
    save_response_screenshot("get_invalid_block_directory", response)
    
    # Accept 404 (not found) as expected, or 200 if somehow valid
    assert response.status_code in [200, 404], f"Expected 200 or 404, got {response.status_code}"

def test_unauthorized_block_directory():
    """Test Case 3: Unauthorized access to block-directory"""
    url = f"{BASE_URL}/wp/v2/block-directory/{{id}}".replace("{{id}}", "test")
    try:
    ...
    save_response_screenshot("unauthorized_block-directory", response)
...
def test_response_schema_block_directory():
    """Test Case 4: Response Schema Validation for block-directory"""
    # First, get list to find a valid identifier
    list_path = "/wp/v2/block-directory"
    list_url = f"{BASE_URL}{list_path}"
    try:
    ...
    save_response_screenshot("schema_block_directory", response)
...
def test_response_content_type_block_directory():
    """Test Case 5: Response Content Type for block-directory"""
    # First, get list to find a valid identifier
    list_path = "/wp/v2/block-directory"
    list_url = f"{BASE_URL}{list_path}"
    try:
    ...
    save_response_screenshot("content_type_block_directory", response)
...
def test_response_structure_block_directory():
    """Test Case 6: Response Structure Validation for block-directory"""
    # First, get list to find a valid identifier
    list_path = "/wp/v2/block-directory"
    list_url = f"{BASE_URL}{list_path}"
    try:
    ...
    save_response_screenshot("structure_block_directory", response)
